...
pub const Reactor = struct {
    ...
    // worker pool
    workers: []Worker,
    worker_pool_job_queue: *SPMCQueue(WorkerJob),
    ...
    fn startThread(
        ...
        // worker pool
        const workers = try allocator.alloc(Worker, options.worker_pool_size_per_io);
        errdefer allocator.free(workers);

        const worker_pool_job_queue = try allocator.create(SPMCQueue(WorkerJob));
        worker_pool_job_queue.* = try SPMCQueue(WorkerJob).init(allocator, options.max_jobs_in_queue);
                    ...
                    while (self.job_queue.tryPop()) |job| {
                        ...
                        switch (job.request) {
                            .http => |http_request| {
                                ...
                                const match_result = self.router.lookup(&http_request.host, http_request.path, http_request.method);
                                ...
                                switch (match_result) {
                                    .match => |match| switch (match.action.executor) {
                                        ...
                                        .worker_pool => { // pass the job over to the worker pool, guaranteed to be a valid request
                                            self.worker_pool_job_queue.tryPush(.{
